...
const getFindBarangKeluarById = async (req,res) => {
    ...
    const sql = `SELECT * FROM barang_keluar WHERE id_barang = ${id_barang}`
...
const postNewBarangKeluar = async (req,res) => {
    // const data ={...req.body} 
    const sql = "INSERT INTO barang_keluar SET ?"
...
const deleteBarangKeluar = async (req,res) => {
    const {id_barang} = req.body
        const sqlSearch = `SELECT * FROM barang_keluar WHERE id_barang =  ${id_barang}`
        const sqlDelete = `DELETE FROM barang_keluar WHERE id_barang =  ${id_barang}`
